# cassiano25Ubc - Desenvolvedor Front-end junior & Designer

## Sobre mim
E aí, tudo bem?
Atualmente no 7º período da faculdade, sou um desenvolvedor em formação focado em desenvolvimento front-end e arquitetura de software. Combino minha forte base em lógica de programação com o interesse por criar interfaces funcionais, eficientes e focadas na experiência do usuário. Tenho facilidade em diagnosticar e otimizar linhas de código, transformando requisitos lógicos em aplicações visuais modernas. Busco evoluir constantemente no ecossistema web, integrando o desenvolvimento de layouts dinâmicos à performance técnica dos sistemas.
## Soft-skills
***Comunicativo, Flexível, Engajado, Comprometido, Proativo, Detalhista, Organizado***

## Tecnologias
[![HTML](https://img.shields.io/badge/HTML-%23e54d26?logo=html5&logoColor=white&style=for-the-badge)](https://developer.mozilla.org/en-US/docs/Web/HTML) [![CSS](https://img.shields.io/badge/CSS-%233d8fc6?logo=css3&logoColor=white&style=for-the-badge)](https://developer.mozilla.org/en-US/docs/Web/CSS) [![JavaScript](https://img.shields.io/badge/JavaScript-%23f6d70b?logo=javascript&logoColor=black&style=for-the-badge)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)  [![Git](https://img.shields.io/badge/Git-%23f34f29?logo=git&logoColor=white&style=for-the-badge)](https://git-scm.com/) [![GitHub](https://img.shields.io/badge/GitHub-%23181717?logo=github&logoColor=white&style=for-the-badge)](https://github.com/) 

## Currículo
Aqui você encontrará informações detalhadas sobre minha trajetória profissional, habilidades e experiências.

[Veja meu currículo aqui]()


## Contato
- **E-mail**: [cassiano25.ubc@gmail.com](cassiano25.ubc@gmail.com)
- **Telefone**: [(31) 9 7138-0857]()
- **linkedin**:[linkedin](www.linkedin.com/in/cassiano-maia-743160208)

